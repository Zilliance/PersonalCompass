...
class ZillianceTextViewController: UIViewController, UIViewControllerTransitioningDelegate {
    ...
    private func setupView() {
        ...
        if let textViewContent = self.feelBetterType {
            self.text(for: textViewContent)
        } else {
            let positiveActivities = (self.compass.positiveActivities.flatMap { $0.title }).joined(separator: ",\n")
            let text = "\(self.compass.concreteStep!) \n\n" + positiveActivities
            self.textView.text = text
        }
...
extension ZillianceTextViewController: UITextViewDelegate {
    ...
    func textViewDidChangeSelection(_ textView: UITextView) {
        ...
        if let _ = self.tappableIndexForRange(range: range, textsList: editableTexts)
        {
    ...
    fileprivate func text(for feelBetterType: FeelBetterType) {
